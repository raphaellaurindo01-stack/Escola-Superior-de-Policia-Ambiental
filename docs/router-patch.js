...
navigate=function(v){
  stopDictation();
  if(v==='courses')return renderCourses();
  if(v==='access')return renderAccess();
  if(v==='course-manager')return renderCourseManager();
...
renderHome=function(){
 ...
 <div class="quick reference-nav"><button onclick="renderCourses()">🎓 Cursos e Estágios</button><button onclick="renderModule('legislacao')">📚 Legislação Ambiental</button><button onclick="renderModule('fauna')">🐾 Fauna</button><button onclick="renderModule('flora')">🌳 Flora</button><button onclick="renderReference('operacoes')">🚒 Operações</button><button onclick="renderEnvironmentalAI()">💡 Consulta Ambiental</button><button onclick="renderReference('anexos')">📎 Anexos</button></div>
 ...
 <article class="card" role="button" tabindex="0" onclick="renderEnvironmentalAI()"><div class="icon">💡</div><h3>Consulta Ambiental</h3><p>Consulte a biblioteca ESPA e fontes oficiais externas quando disponíveis.</p></article>
 <article class="card" role="button" tabindex="0" onclick="renderCourses()"><div class="icon">🎓</div><h3>Central de Cursos e Estágios</h3><p>Consulte o CCE, datas, público-alvo, vagas e períodos de inscrição.</p></article></div>
 ...
 <article class="card" role="button" tabindex="0" onclick="renderChecklists()"><div class="icon">✅</div><h3>Checklists</h3><p>Vistoria geral e conferência de documentos.</p></article><article class="card" role="button" tabindex="0" onclick="renderTools()"><div class="icon">🧮</div><h3>Ferramentas</h3><p>APP, área, coordenadas e cálculos rápidos.</p></article><article class="card" role="button" tabindex="0" onclick="renderBopamb()"><div class="icon">📝</div><h3>Gerador BOPAmb</h3><p>Monte um texto-base com os principais dados da ocorrência.</p></article></div>
...
const originalRenderModule=renderModule;renderModule=function(id){if(id==='reurb')return renderReurb();return originalRenderModule(id)};
