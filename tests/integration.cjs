...
run("navigate('resenha')");const resenhaHtml=node('#content').innerHTML;for(const term of ['Gerador de Resenha Policial','Natureza da ocorrência','Município','Constatação / fatos objetivos','Autores e autuações','Adicionar autor','Enquadramento administrativo','Enquadramento penal','Penalidade aplicada','Medidas administrativas gerais','Atendimento Ambiental','Data do Atendimento Ambiental','Hora do Atendimento Ambiental','Forma de encaminhamento'])assert.ok(resenhaHtml.includes(term),term);assert.ok(!resenhaHtml.includes('Providências na esfera penal'),'campo penal antigo não deve aparecer');
...
console.log('OK: REURB, matriz, estados documentais, busca, ocorrência, enquadramentos administrativo/penal, atendimento ambiental e cache dos scripts.');})().catch(e=>{console.error(e);process.exitCode=1});
