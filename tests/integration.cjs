...
run("navigate('resenha')");const resenhaHtml=node('#content').innerHTML;for(const term of ['Gerador de Resenha Policial','Natureza da ocorrência','Município','Constatação / fatos objetivos','Autores e autuações','Adicionar autor','Medidas administrativas gerais','Atendimento Ambiental','Providências na esfera penal'])assert.ok(resenhaHtml.includes(term),term);
...
console.log('OK: REURB, matriz, estados documentais, busca, ocorrência, gerador estruturado de resenha e cache dos scripts.');})().catch(e=>{console.error(e);process.exitCode=1});
